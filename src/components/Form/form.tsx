...
function GetForm () {
    ...
    return (
        <div className="someform">
         ...
         {/* У тебя сейчас вызывается функция при каждом изменении стейта
         ПОтому что при изменении стейта происходит ререндер
         Ты можешь сохранять результать в переменную и выводить ее сюда
         
         А вызывать функцию через onClick событие по кнопке Искать
         в таком случае input надо заменить на button */}
          <h2>{GetCuadrad()}</h2>
        </div>
    );
}
